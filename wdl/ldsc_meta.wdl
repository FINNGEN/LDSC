...
workflow ldsc_meta {
    ...
    call gather_h2 {
        input:
            name = final_name,
            docker = docker,
            het_jsons = munge_ldsc.het_json,
            het_log = munge_ldsc.het_log
    }
    ...
    if (!only_het)
    {
        ...
        call return_couples {
            input:
                file = meta_fg,
                munged_chunks = munge_ldsc.munged,
                docker = docker
        }
        ...
        scatter (i in range(length(return_couples.couples))) {
            ...
            call multi_rg {
                input:
                    couples = return_couples.couples[i],
...
task return_couples {
    ...
    Array[Array[String]] list = read_tsv(file)
    ...
    command <<<
        ...
        # open input file and parse into groups
        groups = defaultdict(list)
        with open('~{write_tsv(list)}') as i:
            for line in i:
                parts = line.strip().split()
                filename = parts[0]
                group = parts[3] # assuming group is in the 4th column (index 3)
                groups[group].append(filename)

        # create all possible combinations within each group
        group_combinations = {}
        for group, filelist in groups.items():
            unique_files = list(set(filelist))
            # create all combinations within this group
            group_couples = list(itertools.combinations(unique_files, 2))
            # sort them
            group_combinations[group] = sorted([list(elem) for elem in group_couples])

        total_files = sum(len(set(files)) for files in groups.values())
        total_combinations = sum(len(couples) for couples in group_combinations.values())
        ...
        # convert pheno couples to file couples and create chunks (one per group) with absolute paths
        for i, (group, couples) in enumerate(group_combinations.items()):
            with open(f"./chunk_{i}", 'wt') as o, open(f"./paths_{i}", 'wt') as p:
                paths = []
                for c in couples:
                    o.write('\t'.join(c) + '\n')
                    for pheno in c:
                        paths.append(pheno_file_dict[pheno])
                p.write('\n'.join(set(paths)))

        with open('jobs.txt', 'wt') as o:
            o.write(str(len(group_combinations)))
...
task filter_meta {
    ...
    output {
        Array[File] chunk_list = glob("./chunk*")
    }
